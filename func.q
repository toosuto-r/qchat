...
mkct:{[x;y;z] if[2>count r:r where 1&count'[r:" "vs "c"$3_x];:neg[y]@0,ccache[aw?y]"j"$"Please input in format CHATNAME USER1 USER2 USER3... to add users from scratch or CHATNAME -USER1 USER2... to make a new chat with all but the named users from this chat"];
  if[r[0]in ?\:[n;" "]#'n:raze each (6+n ss\:"-name ")_'(n:p where count each(p:system"ps -ef | grep chatter.q")ss\:"chatter.q");
    :neg[y]@0,ccache[aw?y]"j"$"Chat exists and is currently active - quitting chat creation"];
  if[(cn:$[`;raze string md5 "Chat Room: ",r 0]) in key`:.;
    neg[y]@0,ccache[aw?y]"j"$"Chat already exists - taking existing userlist";
...
emji:{[x;y;z] if[not(`$3_"c"$x) in key emdict;:neg[y]@0,ccache[aw?y]"j"$"Unknown emoji - meme deficiency detected."];chat[;y;z]emdict `$"c"$3_x;}
